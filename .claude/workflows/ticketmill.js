...
// Full rationale: docs/architecture/engine-internals.md#ticketmill
    ...
    observations: {
      ...
      items: {
        ...
        properties: {
          ...
          // Full rationale: docs/architecture/engine-internals.md#later-fix-pr
...
// the issue itself across a run boundary.
// Full rationale: docs/architecture/engine-internals.md#gate-state-issue
...
// comment.
// Full rationale: docs/architecture/engine-internals.md#buildgatestatepayload
...
// Full rationale: docs/architecture/engine-internals.md#selectgatestate
...
// Full rationale: docs/architecture/engine-internals.md#diffgatestateintent
...
// attachGateStateBlocks: per-preflight normalizer AND real-data join,
// mirroring attachEngineOwnedIntentional's shape (:3023) -- guarantees every
// preflight carries all four gate-state PREFLIGHT_SCHEMA fields
// (gate_state_blocks, gate_state_read_ok, gate_state_total_comments,
// gate_state_trust) UNCONDITIONALLY.
// Full rationale: docs/architecture/engine-internals.md#attachgatestateblocks
...
// Full rationale: docs/architecture/engine-internals.md#consolidation
...
// reconcileGroups: make LIVE claimed preflights authoritative over group
// membership.
// Full rationale: docs/architecture/engine-internals.md#reconcilegroups
...
// deriveUnits: the final translation from "reconciled groups" + "live
// preflights" to the array runPool() actually iterates.
// Full rationale: docs/architecture/engine-internals.md#deriveunits
...
// computeLanes: pure reducer (issue #1, lane scheduling) that groups
// deriveUnits()'s output into lanes — sets of unit INDICES that must run
// serially (one worker draining the lane in order) instead of racing.
// Full rationale: docs/architecture/engine-internals.md#computelanes
...
// applyRealRunCollapseGuard: pure reducer (issue #1, lane scheduling) — a
// final, run-time safety net called immediately before runPool()'s real drain
// (dry-run separately previews lanes read-only, before claims settle — see the
// DRY_RUN block).
// Full rationale: docs/architecture/engine-internals.md#applyrealruncollapseguard
...
// counts as one.
// Full rationale: docs/architecture/engine-internals.md#isbudgetexhaustederror
...
// issue #163 wired in quality) ----- Per-gate (the two per-issue contrarian
// gates 'approach'/'plan', the per-task-plus-per-PR-fix-round 'quality' code-
// review gate, plus the per-task 'pr-review' merge gate) tally of finding
// counts, severity mix, and how that gate ITERATION resolved:
// Full rationale: docs/architecture/engine-internals.md#gate-findings-tally
...
// (quality- fix, pr-fix, test-quality-fix) — the ONE place that decides what a
// fix agent sees of a review, so structured findings and prose comments never
// drift apart across the three sites.
// Full rationale: docs/architecture/engine-internals.md#findingsblock
...
// Pure aggregation of per-issue/per-stage token deltas into per-issue, per-
// model, and per-stage subtotals, plus a finished markdown "## Token Usage"
// section — all math done here in JS, never delegated to an LLM.
// Full rationale: docs/architecture/engine-internals.md#pure-aggregation-per
function aggregateTokens(results, spent, concurrency, byStage, poolSpend) {
  ...
  // reconcile_error (issue #90, re-scoped by #111):
  // Full rationale: docs/architecture/engine-internals.md#reconcile-error
...
// see.
// Full rationale: docs/architecture/engine-internals.md#friction-weights
...
// Full rationale: docs/architecture/engine-internals.md#computefriction
...
// backstop.
// Full rationale: docs/architecture/engine-internals.md#engine-owned-gate
...
// commit SHAs):
// Full rationale: docs/architecture/engine-internals.md#probecommitshas
...
// runBrowserCheck:
// Full rationale: docs/architecture/engine-internals.md#merge-auto-resolve
...
// =============================================================================
// PROPOSECONSOLIDATION (Select-phase judgment gate; ABOVE the harness split
// like implementIssue/reviewAndMerge, so tests/harness.js can drive it with a
// scripted agent()).
// Full rationale: docs/architecture/engine-internals.md#proposeconsolidation
...
// READ IDIOM is deliberately NOT that one:
// Full rationale: docs/architecture/engine-internals.md#fetchgatestateblocks
...
// verifyGateState (issue #166, task 4): the Report-phase self-validation sweep
// that proves post -> GitHub -> read -> parse for gate state in one run.
// Full rationale: docs/architecture/engine-internals.md#verifygatestate
...
async function reviewAndMerge(ctx) {
  ...
  for (let iter = 1; iter <= MAX_PR_REVIEW_ITERATIONS && !approved; iter++) {
    ...
    // gate_findings tally (issue #91, retyped by issue #162):
    // Full rationale: docs/architecture/engine-internals.md#gate-findings-tally-2
...
// Bounded worker pool (issue-level concurrency; agent-level pool is capped by
// the harness).
// Full rationale: docs/architecture/engine-internals.md#bounded-worker-pool
...
// runPool()'s own module comment, but looked up by issue number here rather
// than position so a future reordering can never silently mismatch).
// Full rationale: docs/architecture/engine-internals.md#buildissueshaperows
...
// buildRunRecord (issue #86): assemble the FULL, untruncated machine-readable
// record for a run.
// Full rationale: docs/architecture/engine-internals.md#buildrunrecord
...
// ledger lines, i.e.
// Full rationale: docs/architecture/engine-internals.md#buildtrustedpfbands
...
// in this repo, where every tier touches workflows/ticketmill.js).
// Full rationale: docs/architecture/engine-internals.md#computelaterbatchfix
...
// gradeFromObservation (issue #92): the deterministic grade decision.
// Full rationale: docs/architecture/engine-internals.md#gradefromobservation
...
// pure function is the only place that decides a flag).
// Full rationale: docs/architecture/engine-internals.md#computerevisitrisk
...
// deriveNegativeOutcomeEvents (issue #93 quality-fix, iteration 1 — code
// review verdict changes_requested):
// Full rationale: docs/architecture/engine-internals.md#derivenegativeoutcomeevents
...
// ---- Select: consolidation gate (judgment call — see the
// PROPOSECONSOLIDATION module comment above the harness split for the full
// design).
// Full rationale: docs/architecture/engine-internals.md#select-consolidation-gate
...
if (DRY_RUN) {
  ...
  // matches the unit a real run would actually drain.
  // Full rationale: docs/architecture/engine-internals.md#cost-estimate-preview
