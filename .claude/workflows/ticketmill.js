...
async function reviewAndMerge(ctx) {
  ...
  const merge = await stage(ctx, 'merge', [
    ...
    '',
    '1. Preflight: gh pr view ' + ctx.pr + ' --repo ' + REPO + ' --json state,mergeable,mergeStateStatus',
    '   If NOT open+mergeable (conflict etc.): DO NOT merge and do NOT post the Implementation Complete comment',
    '   below; return status=blocked with the reason.',
