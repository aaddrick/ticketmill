...
- **Weakened incident machinery**: diffs that simplify away the stub-task guard, settled-decisions ledger, handoff notes, comment markers, claim label-safety, browser lock, or degrade windows. Each answers a documented incident (docs/architecture/failure-semantics.md table); removal needs explicit justification, not cleanup instinct.
- **Docs/constants drift**: README and docs/architecture/ state cap values, model policy, and pipeline shape. A diff changing `MAX_*` constants, stage order, or the pipeline flow without updating docs ships lies to users. The pipeline diagrams are D2 sources under `docs/diagrams/` with checked-in SVGs beside them; a stage-order change needs the `.d2` edited and `docs/diagrams/render.sh` re-run in the same commit, or the committed SVGs go stale.
...
- **`agentType` in engine agent() calls**: the engine uses persona-by-reference only; see docs/architecture/agents-and-models.md "One agent mechanism".
