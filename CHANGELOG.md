...
Documentation only. Splits `docs/ARCHITECTURE.md` into topic files and moves
README's reference sections into `docs/`, closing #154 and #155.

- `docs/architecture/` (new, ten files) replaces the 1310-line
  `docs/ARCHITECTURE.md`, generated by a manifest-driven script rather than
  hand-copied so every moved line is provenance-hashed. `pipeline.md` folds
  `## Pipeline` into its own H1 and carries the overview picture, the
  provenance paragraph, the legend, and all five phase sections, which is
  where all thirteen diagram-path rewrites land; `failure-semantics.md`
  emits its two segments out of source order; the other seven files get a
  synthetic H1 and a one-sentence lede above their first demoted heading.
  `docs/ARCHITECTURE.md` is now a two-line stub pointing at
  `docs/architecture/index.md`.
- `tests/architecture-provenance.test.js` (new, no git shell-out) reassembles
  every moved segment from the committed output, reverses the recorded
  rewrites, and hashes the result against digests pinned in
  `tests/fixtures/architecture-split.json`. A red run means moved prose was
  hand-edited after the split, not a digest to update.
- Every referrer of the old `docs/ARCHITECTURE.md` is repointed at its
  specific successor file: `docs/diagrams/{AGENTS,CLAUDE}.md` and
  `render.sh` now cite `pipeline.md` alone, and the four `.claude/agents/*.md`
  charters cite whichever topic file now holds the section they used to
  reference.
- `README.md` (340 lines) trims to 123: Requirements, Run options, Watching a
  run, Follow-up issues, Overlapping batches, Resuming an interrupted run,
  Troubleshooting, How agents work, and Profile reference move verbatim into
  seven new `docs/*.md` files, plus a new `docs/skills.md` authored from the
  four `skills/*/SKILL.md` frontmatters. README keeps Install, Quickstart,
  "What makes it trustworthy," Repo layout, Author, and License, with a
  one-line pointer into `docs/` wherever a section moved out.
- `tests/docs-links.test.js` (new) checks relative-link resolution, two-hop
  reachability from `docs/index.md`, and AGENTS/CLAUDE byte-identity across
  every tracked `.md` file. `tests/readme-docs-census.test.js` (new) pins
  every one of README's original fifteen H2 sections to appear exactly once
  across README.md plus docs/*.md.
- Fixed a real drift while moving the profile table: `simplify_globs` unset
  or `null` runs the simplifier against every changed file (fail-open); it
  does not skip the stage the way `docblock_globs`/`docs_dir` do.
  `docs/profile.md` documents the correction instead of folding it in
  silently.
