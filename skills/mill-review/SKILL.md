...
repo's blanket `logs/` gitignore rule, and committing a summary of untracked,
per-host data would be the one tracked file in that tree. This is a known,
called-out deviation from the issue's literal wording, decided during
approach-challenge (iteration 2, "sound_with_caveats") and carried into the
plan explicitly as vetoable. If the batch reviewer disagrees, the fix is
narrow: drop the `logs/` ignore for this one path (`!logs/ticketmill/dashboard.md`)
and add `git add -f` in Step 7c — do not reinterpret this as a bug.
-->
...
as a shortcut — the same skill runs against ticketmill's own ledger and
...
- **A date range `YYYY-MM-DD..YYYY-MM-DD`** — derive each run's date from its
  `batch_branch` field (format `Batch_YYYY-MM-DD_HHMMSS`; take the date
...
otherwise scan the directory broadly — legacy `summary-*.json` /
...
2. **`LOGS/runs/<run_tag>.json`** for every run_tag selected in step 1 above
   (skip a run whose file is missing — note it as a gap, don't error). This
   is the detailed per-run record: `completeness`, `friction_churn`,
...
3. **`LOGS/outcomes.jsonl`** — one JSON object per `(run_tag, batch_pr,
   ...
   appear more than once as its grade evolves (e.g. `pending` → `clean`); the
   *last* line for a given `(run_tag, issue)` pair wins, matching the
   engine's own read convention. Build the outcome scorecard from the
...
4. **`LOGS/process-retrospective.md`** — read the whole file, but pull three
   ...
   - The **Lane Prediction Accuracy** table (`| Date | Issues | Coverage |
     Precision |`) — read its rows verbatim, do not recompute coverage or
...
- The winning outcome-grade per issue in this run, from Step 3.3 (or `null`
  per issue if no grading event exists yet — most current runs are entirely
  `pending`).
...
Set `MIN_RUNS = 5`. This is the default; there is no profile field for it —
name it plainly in the output so a human reading the highlights or dashboard
knows it's a fixed threshold, not a computed one.
...
2. **`N < MIN_RUNS`**: this is the *below-threshold* path.
   ...
   - Attach the **issue-class heterogeneity caveat** explicitly: at low
     ...
     read is not a trend, it's an artifact of which issues happened to land
     in which run. Say this once, prominently, not as a footnote.
...
4. **Lane precision is gated on its own**, independent of the JSONL-derived
   ...
   there isn't yet enough history for a trend read; do not extrapolate.
...
1. **Outcome scorecard.** Bucket each `(run, issue)` winning grade from Step
   ...
   outcomes recorded, all still pending, no verdict yet" — rather than
   printing a 0% or 100% held rate that implies a verdict that doesn't exist.
...
3. **Chronic bumpy stages / issue classes.** Using each run's own
   `friction_churn.friction.top_stages` / `top_issues` (already computed —
   ...
   one run is a data point; a stage that tops the list in three of five runs
   is chronic — call out the latter, cite the run_tags.
4. **Churn hotspots & re-fix chains.** Union `friction_churn.churn.hotspots`
   ...
   list, concatenated and deduplicated by file path / chain identity — not
...
6. **Stale / contradicted learnings.** Two sources, both from
   ...
   **Deprecated Learnings** section — list them; (b) any **Active Learnings**
   bullet that contains its own internal "Superseded", "Contradicted",
   "Fixed", or "Reconfirmed... not the...previously observed" language —
   these are learnings the file keeps active but has partially walked back
   inline rather than deprecating outright. Flag both kinds; don't silently
   drop either.
7. **Ranked "what I'd change next."** A short (5–7 item) judgment list, each
   ...
   evidence supports it — a chronic stage confirmed across most of the
   window outranks a one-off observation from a single run. This is the one
...
Publish it via the Artifact tool (favicon required; this is a data/analytics
artifact, pick accordingly). If the Artifact tool isn't available in this
